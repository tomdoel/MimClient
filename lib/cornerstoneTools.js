...
(function($, cornerstone, cornerstoneTools) {
    ...
    function chooseLocation(e, eventData) {
        ...
        var sourceImagePlane = cornerstoneTools.metaData.get('imagePlane', baseLayer.image.imageId);
...
// Begin interactive edit tool

(function($, cornerstone, cornerstoneMath, cornerstoneTools) {

    'use strict';

    var toolType = 'scribble';
    ...
    function addPoint(eventData) {
        ...
        var points = toolData.data[toolData.data.length-1].points;
        var point = {
    ...
    function CreatePointSet(eventData, color) {
        $(eventData.element).on('CornerstoneToolsMouseDrag', mouseDragCallback);
        $(eventData.element).on('CornerstoneToolsMouseUp', mouseUpCallback);

        var pointSetData = {
            points: [], colour: color, visible: true, active: true, handles: [],
        };
        cornerstoneTools.addToolState(eventData.element, toolType, pointSetData);
    }
    ...
    function endDrawing(eventData) {
        ...
        $(eventData.element).off('CornerstoneToolsMouseMove', mouseDragCallback);
    ...
    function mouseDownCallback(e, eventData) {
        if (cornerstoneTools.isMouseButtonEnabled(eventData.which, e.data.mouseButtonMask)) {
            ...
            CreatePointSet(eventData, color);
            addPoint(eventData);
            return false;            
        }
    }
    ...
    function onImageRendered(e, eventData) {
        ...
        var color;
        var lineWidth = 10; //cornerstoneTools.toolStyle.getToolWidth();
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            if (points) {
                ...
                for (var j = 0; j < points.length; j++) {
                    var point = points[j];
                    var lineCanvas = cornerstone.pixelToCanvas(eventData.element, point);
                    
                    if (j === 0) {
                        context.moveTo(lineCanvas.x, lineCanvas.y); 
                    } else {
                        context.lineTo(lineCanvas.x, lineCanvas.y);
                    }
    ...
    function enable(element) {
        $(element).off('CornerstoneToolsMouseDown', mouseDownCallback);
        $(element).off('CornerstoneToolsMouseUp', mouseUpCallback);
        $(element).off('CornerstoneToolsMouseMove', mouseDragCallback);
        $(element).off('CornerstoneImageRendered', onImageRendered);
    ...
    // module/private exports
    cornerstoneTools.interactiveedit = {
        enable: enable, disable: disable, activate: activate, deactivate: deactivate, getConfiguration: getConfiguration, setConfiguration: setConfiguration
    };
    
})($, cornerstone, cornerstoneMath, cornerstoneTools);
    
// End interactive edit tool
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function getOrientationMarkers(element) {
        ...
        var baseLayer = enabledElement.layers[0];
        var imagePlaneMetaData = cornerstoneTools.metaData.get('imagePlane', baseLayer.image.imageId);
...
(function(cornerstone, cornerstoneTools) {
    ...
    function scrollToIndex(element, newImageIdIndex) {
        ...
        function overlayDoneCallback(image) {
            console.log('interaction done: ' + image.imageId);
            if (stackData.currentImageIdIndex === newImageIdIndex) {
                cornerstone.displayImageAndOverlay(element, undefined, image, viewport);
                if (endLoadingHandler) {
                    endLoadingHandler(element);
                }
            }
        ...
        var newOverlayImageId = stackData.overlayImageIds[newImageIdIndex];
        ...
        requestPoolManager.addRequest(element, newOverlayImageId, type, overlayDoneCallback, failCallback);
        requestPoolManager.startGrabbing();

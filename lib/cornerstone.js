...
(function (cornerstone) {
    ...
    function getPixels(element, x, y, width, height) {
        ...
        var enabledElement = cornerstone.getEnabledElement(element);
        var slope = enabledElement.image.slope;
        var intercept = enabledElement.image.intercept;
...
(function (cornerstone) {
    ...
    function calculateTransform(enabledElement, scale) {
        ...
        transform.translate(baseLayer.canvas.width/2, baseLayer.canvas.height / 2);

        //Apply the rotation before scaling for non square pixels
        var angle = baseLayer.viewport.rotation;
        if(angle!==0) {
            transform.rotate(angle*Math.PI/180);
        }

        // apply the scale
        var widthScale = baseLayer.viewport.scale;
        var heightScale = baseLayer.viewport.scale;
        if(baseLayer.image.rowPixelSpacing < baseLayer.image.columnPixelSpacing) {
            widthScale = widthScale * (baseLayer.image.columnPixelSpacing / baseLayer.image.rowPixelSpacing);
        }
        else if(baseLayer.image.columnPixelSpacing < baseLayer.image.rowPixelSpacing) {
            heightScale = heightScale * (baseLayer.image.rowPixelSpacing / baseLayer.image.columnPixelSpacing);
        }
        ...
        transform.translate(baseLayer.viewport.translation.x, baseLayer.viewport.translation.y);
        ...
        if(baseLayer.viewport.hflip) {
            transform.scale(-1,1);
        }

        if(baseLayer.viewport.vflip) {
            transform.scale(1,-1);
        }

        // translate the origin back to the corner of the image so the event handlers can draw in image coordinate system
        transform.translate(-baseLayer.image.width / 2 , -baseLayer.image.height/ 2);
        return transform;
    }
...
(function (cornerstone) {
    ...
    function getRenderCanvas(enabledElement, layer, image, invalidated)
    {
    ...
    function renderColorImage(enabledElement, layer, invalidated) {
        ...
        var renderCanvas = getRenderCanvas(enabledElement, layer, image, invalidated);

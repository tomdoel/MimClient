    ...
    function disable(element) {
        ...
        for(var i=0; i < enabledElements.length; i++) {
            if(enabledElements[i].element === element) {
                ...
                enabledElements[i].layers.forEach(function(layer) {
                    enabledElements[i].element.removeChild(layer.canvas);
                });
...
(function ($, cornerstone) {
    ...
    function displayImage(element, image, viewport) {
        displayImageAndOverlay(element, image, undefined, viewport);
    }
    
    function displayImageAndOverlay(element, image, overlay, viewport) {
        ...
        if (enabledElement.layers[0] && image) {
            enabledElement.layers[0].image = image;
           enabledElement.image = image;
        }
        if (enabledElement.layers[1] && overlay) {
           enabledElement.layers[1].image = overlay;
        }

        enabledElement.layers.forEach(function(layer) {
            if (layer.viewport === undefined && layer.image) {
                layer.viewport = cornerstone.internal.getDefaultViewport(layer.canvas, layer.image);
            }
        });
        ...
        if (viewport) {
            for(var attrname in viewport)
            {
                if (viewport[attrname] !== null) {
                    enabledElement.layers.forEach(function(layer) {
                        layer.viewport[attrname] = viewport[attrname];
                    });
                }
            }
        }
    ...
    cornerstone.displayImageAndOverlay = displayImageAndOverlay;
}($, cornerstone));
/**
...
(function (cornerstone) {
    ...
    function enable(element) {
        ...
        var overlayLayerEnabledElement = {
            layerId: 1,
            globalAlpha: 0.5,
        ...
        var el = {
            layers: [baseLayerEnabledElement, overlayLayerEnabledElement],
            element: element,
            image : undefined, // will be set once image is loaded
...
(function (cornerstone) {
    ...
    function getEnabledElementsByImageId(imageId) {
        ...
        enabledElements.forEach(function(enabledElement) {
            var found = false;
            enabledElement.layers.forEach(function(layer) {
                if(layer.image && layer.image.imageId === imageId) {
                    found = true;
                }
            });
            if (found) {
...
(function (cornerstone) {
    ...
    function fitToWindow(element)
    {
        ...
        enabledElement.layers.forEach(function(layer) {
            if (layer.image) {
                var defaultViewport = cornerstone.internal.getDefaultViewport(layer.canvas, layer.image);
                layer.viewport.scale = defaultViewport.scale;
                layer.viewport.translation.x = defaultViewport.translation.x;
                layer.viewport.translation.y = defaultViewport.translation.y;
                layer.viewport.rotation = defaultViewport.rotation;
                layer.viewport.hflip = defaultViewport.hflip;
                layer.viewport.vflip = defaultViewport.vflip;
            }
        });
        cornerstone.updateImage(element);
    }
...
(function ($, cornerstone) {
    ...
    function getDefaultViewportForImage(element, image) {
        var enabledElement = cornerstone.getEnabledElement(element);
        var baseLayer = enabledElement.layers[0];
        var viewport = cornerstone.internal.getDefaultViewport(baseLayer.canvas, image);
        return viewport;
    }
...
(function (cornerstone) {
    ...
    function getPixels(element, x, y, width, height) {
        ...
        var enabledElement = cornerstone.getEnabledElement(element);
        var baseLayer = enabledElement.layers[0];
        var slope = baseLayer.image.slope;
        var intercept = baseLayer.image.intercept;
...
(function (cornerstone) {
    ...
    function getViewport(element) {
        var enabledElement = cornerstone.getEnabledElement(element);
        var baseLayer = enabledElement.layers[0];
        var viewport = baseLayer.viewport;
...
(function (cornerstone) {
    ...
    function calculateTransform(layer, scale) {
        var transform = new cornerstone.internal.Transform();
        transform.translate(layer.canvas.width/2, layer.canvas.height / 2);

        //Apply the rotation before scaling for non square pixels
        var angle = layer.viewport.rotation;
        if(angle!==0) {
            transform.rotate(angle*Math.PI/180);
        }

        // apply the scale
        var widthScale = layer.viewport.scale;
        var heightScale = layer.viewport.scale;
        if(layer.image.rowPixelSpacing < layer.image.columnPixelSpacing) {
            widthScale = widthScale * (layer.image.columnPixelSpacing / layer.image.rowPixelSpacing);
        }
        else if(layer.image.columnPixelSpacing < layer.image.rowPixelSpacing) {
            heightScale = heightScale * (layer.image.rowPixelSpacing / layer.image.columnPixelSpacing);
        }
        ...
        transform.translate(layer.viewport.translation.x, layer.viewport.translation.y);
        ...
        if(layer.viewport.hflip) {
            transform.scale(-1,1);
        }

        if(layer.viewport.vflip) {
            transform.scale(1,-1);
        }

        // translate the origin back to the corner of the image so the event handlers can draw in image coordinate system
        transform.translate(-layer.image.width / 2 , -layer.image.height/ 2);
        return transform;
    }
...
(function ($, cornerstone) {
    ...
    function drawImage(enabledElement, invalidated) {
        ...
        enabledElement.layers.forEach(function(layer) {
            if (layer.image) {
                layer.image.render(layer, invalidated);
            }
        });

        var baseLayer = enabledElement.layers[0];
        var baseLayercontext = baseLayer.canvas.getContext('2d');
        ...
        var eventData = {
            ...
            image : baseLayer.image,
            enabledElement : enabledElement,
            canvasContext: baseLayercontext,
            renderTimeInMs : diff
        };
...
(function (cornerstone) {
    ...
    function getTransform(enabledElement)
    {
        ...
        var transform = cornerstone.internal.calculateTransform(enabledElement.layers[0]);
        return transform;
    }
...
(function (cornerstone) {
    ...
    function doesImageNeedToBeRendered(layer, image)
    {
    ...
    function getRenderCanvas(layer, image, invalidated)
    {
        // apply the lut to the stored pixel data onto the render canvas

        if(layer.viewport.voi.windowWidth === layer.image.windowWidth &&
            layer.viewport.voi.windowCenter === layer.image.windowCenter &&
            layer.viewport.invert === false)
        {
        ...
        {
            if(doesImageNeedToBeRendered(layer, image) === false && invalidated !== true) {
            ...
            // get the lut to use
            var colorLut = getLut(image, layer.viewport);
     ...
     */
    function renderColorImage(layer, invalidated) {
        ...
        context.globalAlpha = layer.globalAlpha;
...
//        context.fillRect(0,0, layer.canvas.width, layer.canvas.height);
        ...
        var renderCanvas = getRenderCanvas(layer, image, invalidated);
    ...
    cornerstone.getRenderCanvas = getRenderCanvas;
...
(function (cornerstone) {
    ...
    function getRenderCanvas(layer, image, invalidated)
    {
        ...
        // get the lut to use
        var lut = getLut(image, layer.viewport, invalidated);
     ...
     * @param invalidated - true if pixel data has been invaldiated and cached rendering should not be used
     */
    function renderGrayscaleImage(layer, invalidated) {
...
//        context.fillRect(0,0, layer.canvas.width, layer.canvas.height);
    ...
    cornerstone.lastRenderedViewport = lastRenderedViewport;
...
//        // clear the canvas
//        context.fillStyle = 'black';
//        context.fillRect(0,0, baseLayer.canvas.width, baseLayer.canvas.height);
//
//        // turn off image smooth/interpolation if pixelReplication is set in the viewport
//        if(baseLayer.viewport.pixelReplication === true) {
//            context.imageSmoothingEnabled = false;
//            context.mozImageSmoothingEnabled = false; // firefox doesn't support imageSmoothingEnabled yet
//        }
//        else {
...
//
//        // save the canvas context state and apply the viewport properties
//        cornerstone.setToPixelCoordinateSystem(enabledElement.layers[0], context);
//
...
//        if(baseLayer.viewport.voi.windowWidth === baseLayer.image.windowWidth &&
//            baseLayer.viewport.voi.windowCenter === baseLayer.image.windowCenter &&
//            baseLayer.viewport.invert === false)
//        {
...
(function (cornerstone) {
    ...
    function resize(element, fitToWindow) {
        ...
        enabledElement.layers.forEach(function(layer) {
            setCanvasSize(element, layer.canvas);        
        });
...
(function (cornerstone) {
     ...
     */
    function setToPixelCoordinateSystem(layer, context, scale)
    {
        if(layer === undefined) {
            throw "setToPixelCoordinateSystem: parameter layer must not be undefined";
        }
        if(context === undefined) {
            throw "setToPixelCoordinateSystem: parameter context must not be undefined";
        }

        var transform = cornerstone.internal.calculateTransform(layer, scale);
...
(function (cornerstone) {
    ...
    function setViewport(element, viewport) {
        ...
        enabledElement.layers.forEach(function(layer) {
            if (layer.viewport) {
                layer.viewport.scale = viewport.scale;
                layer.viewport.translation.x = viewport.translation.x;
                layer.viewport.translation.y = viewport.translation.y;
                layer.viewport.voi.windowWidth = viewport.voi.windowWidth;
                layer.viewport.voi.windowCenter = viewport.voi.windowCenter;
                layer.viewport.invert = viewport.invert;
                layer.viewport.pixelReplication = viewport.pixelReplication;
                layer.viewport.rotation = viewport.rotation;
                layer.viewport.hflip = viewport.hflip;
                layer.viewport.vflip = viewport.vflip;

                // prevent window width from being < 1
                if(layer.viewport.voi.windowWidth < 1) {
                    layer.viewport.voi.windowWidth = 1;
                }
                // prevent scale from getting too small
                if(layer.viewport.scale < 0.0001) {
                    layer.viewport.scale = 0.25;
                }

                if(layer.viewport.rotation===360 || layer.viewport.rotation===-360) {
                    layer.viewport.rotation = 0;
                }
            }            
        });
